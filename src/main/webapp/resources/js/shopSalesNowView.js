...
$(function(){
	...
	todaysPrice = $('#todaysPrice').val();
	userProductSize = $('#userProductSize').val();
	priceTotal = Math.floor(todaysPrice) - Math.floor(todaysPrice*0.05) - Math.floor(todaysPrice*0.03);
	
	console.log('productId', productId);
	console.log('todaysPrice', todaysPrice);
	...
	$.ajax({
		...
		success: function(data){
			...
			$('.shopDetail-content2').append(
				$('<dl/>', {}).append(
					$('<dt/>', {text:'검수비'}),
					$('<dd/>', {text:Math.floor(todaysPrice * 0.05).toLocaleString() + '원'})
				),
				$('<dl/>', {}).append(
					$('<dt/>', {text:'판매 수수료'}),
					$('<dd/>', {text:Math.floor(todaysPrice * 0.03).toLocaleString() + '원'})
				),
				$('<dl/>', {}).append(
					$('<dt/>', {text:'배송비'}),
					$('<dd/>', {text:'무료'})
...
$(document).on('click', '.wrap-up-sell', function(){
	...
	$.ajax({
		...
		success: function(){
			...
			$.ajax({
				type: 'POST',
				url: '/shoeCream/shop/insertAdminPurchaseData',
				data: {
					productId: productId,
					status: 0,
					addressId: 20,
				...
				success: function(){
					$.ajax({
						type: 'POST',
						url: '/shoeCream/shop/getRecentlyAddedSalesId',
						success: function(data){
							lastSalesId = data;
							console.log("lastSalesId", lastSalesId);
							
							
							$.ajax({
								type: 'POST',
								url: '/shoeCream/shop/getRecentlyAddedPurchaseId',
								success: function(data){
									lastPurchaseId = data;
									console.log("lastPurchaseId", lastPurchaseId);
									
							
							
									$.ajax({
										type: 'POST',
										url: '/shoeCream/shop/insertCheckData',
										data: {
											productId: productId,
											salesId: lastSalesId,
											checkState: 0,
										...
										success: function(){
											...
											$.ajax({
												type: 'POST',
												url: '/shoeCream/shop/getRecentlyAddedCheckId',
												success: function(data){
													lastCheckId = data;
														...
														success: function(){
															$.ajax({
																...
																success: function(data){
																	...
																	$.ajax({
																		type: 'POST',
																		url: '/shoeCream/shop/insertDealData',
																		data: {
																		...
																		success: function(){
																			...
																			$.ajax({
																				type: 'POST',
																				url: '/shoeCream/shop/updateSalesStatus',
																				data: {
																					salesId: lastSalesId,
																					status: 1
																				},
																				success: function(){
																					swal('판매하기 성공', '' , 'success')
																					.then(function(){
																						location.href='/shoeCream/my/selling?pg=1'
																					});//end swal
																				},
																				error: function(err){
																					alert('updateSalesStatus 실패');
																					console.log(err);
																				}
																			});//end ajax updateSalesStatus
																		},
																		error: function(err){
																			alert('deal 데이터 생성 실패');
																			console.log(err);
																		}
																	});//end ajax deal
																},
						...
						error: function(err){
							alert('sales seq 가져오기 실패');
							console.log(err);
						}
					});//end ajax
				},
				error: function(err){
					alert('purchase 데이터 생성 실패');
					console.log(err);
				}
			});//end ajax purchase
		},
