...
$(function(){
	...
	lowestPrice = Number( $('#lowestPrice').val() );
	salesId = $('#salesId').val();
	...
	$.ajax({
		...
		success: function(data){
			...
			$('.shopDetail-content').append(
				...
				$('<dl/>', {}).append(
					$('<dt/>', {text:'판매 희망가'}),
					$('<dd/>', {text:Math.floor(lowestPrice).toLocaleString() + '원'})
				)
			);
			...
			$('.shopDetail-content2').append(
				$('<dl/>', {}).append(
					$('<dt/>', {text:'구매 수수료'}),
					$('<dd/>', {text: Math.floor(lowestPrice*0.02).toLocaleString() + '원'})
				),
				$('<dl/>', {}).append(
					$('<dt/>', {text:'배송비'}),
					$('<dd/>', {text:'무료'})
			...
			$('.shopDetail-content3').append(
				$('<dl/>', {}).append(
					$('<dt/>', {text:'총 정산금액'}),
					$('<dd/>', {text: (lowestPrice + Math.floor(lowestPrice*0.02)).toLocaleString() + '원'})
				),
...
$(document).on('click', '.wrap-up-purchase', function(){
	...
	var IMP = window.IMP; // 생략가능
		...
		IMP.request_pay({
			 ...
			 */
			name: productName,
			amount: (lowestPrice + Math.floor(lowestPrice*0.02)),
						...
						success: function(){
							...
							$.ajax({
								...
								success: function(data){
									...
									$.ajax({
										type: 'POST',
										url: '/shoeCream/shop/insertPurchaseData',
										data: {
											productId: productId,
											status: 0,
																						...
																						success: function(){
																							...
																							$.ajax({
																								...
																								success: function(){
																									...
																									$.ajax({
																										type: 'POST',
																										url: '/shoeCream/shop/updatePurchaseStatus',
																										data: {
																											purchaseId: lastPurchaseId,
																											status: 1
																										},
																										success: function(){
																											swal('구입이 완료 되었습니다', '' , 'success').then(function(){
																												location.href='/shoeCream/index.jsp'
																											});//end swal
																										},
																										error: function(err){
																											alert('updatePurchaseStatus 실패');
																											console.log(err);
																										}
																									});//end ajax updatePurchaseStatus
																								},
																								error: function(err){
